...
② 写时强制备份★  写库前**自动**生成一份新的回滚点 `data/backup/backup-<ts>.json`（由 `backupBeforeImport()` 内部完成，`mkdirSync recursive` 重建目录后 `writeFileSync` 写入）；**备份写入失败则整体中止导入并返回 500「导入前备份失败，为保护数据已中止导入」，零写入**（用户 q-2 拍板的落地方式：写时强制备份，而非要求事先已存在备份）
...
| 500 | 导入前备份失败，为保护数据已中止导入 | `backupBeforeImport()` 写 `data/backup/backup-<ts>.json` 失败（用户 q-2 拍板：写时强制备份，失败即中止，零写入） |
...
sequenceDiagram
    ...
    alt 文件 > 5MB 或非 xlsx
        ...
        API->>API: 写时强制备份<br/>backupBeforeImport() 写 data/backup/backup-<ts>.json
        alt 备份写入失败
            API-->>UI: 500 {error:'导入前备份失败，为保护数据已中止导入'}
            UI->>A: 错误提示，零写入
        else 备份成功
            ...
            alt errors.length > 0 或 行数 > 2000
                ...
                Note over API: 复用本次写时生成的备份 backup-<ts>.json（内部步骤，无需前端事先备份）
...
6. `server.js` 新增 `backupBeforeImport()`：`readAllPaged` 三集合 → 写 `data/backup/backup-<ts>.json`（目录不存在则 `mkdirSync recursive`）。该函数**既**由「数据管理」面板「备份全量JSON」按钮调用（可选本地额外备份），**也**作为导入流程的**内部步骤**在写库前自动执行（用户 q-2 拍板：写时强制备份，不再作前置存在校验）。
7. `server.js` 新增 `POST /api/import/xlsx`（admin + multer memoryStorage 5MB）：**先写时强制备份**（`backupBeforeImport()` 内部生成 `data/backup/backup-<ts>.json`，失败则返回 500「导入前备份失败，为保护数据已中止导入」并零写入）→ 两阶段执行（先全量校验，全通过才写库）；`addItem` 逐条写；ID 带批次序号；**严禁 `writeAll('orders')`**；挂 MulterError → 400 JSON 的错误中间件。返回摘要含 `backupFile`。
...
5. **P0-11**：`switchPage()`（L290）admin 看板 header 增加「数据管理」按钮；新增 `dataMgrModal`（导出订单Excel / 下载导入模板 / 导入历史数据 / 备份全量JSON 四项）+ 隐藏 `<input type="file" accept=".xlsx">`；新增 `importResultModal` 展示摘要（新增/跳过/警告/失败 + 默认密码账号提示 + 错误行号列表）；**「备份全量JSON」为可选的本地额外备份工具**（调用 `backupBeforeImport()` 落 `data/backup/backup-<ts>.json`），**不再是导入的前置条件**（写时强制备份已在导入路由内部完成），导入按钮始终可用。上传前 `confirm` 二次确认 + loading 态防重复提交。
...
| **B4** | 导入备份是否强制 | **强制**（用户 q-2 拍板）：采用**写时强制备份**落地——每次 `POST /api/import/xlsx` 在写库前都由 `backupBeforeImport()` **自动生成**一份新的 `data/backup/backup-<ts>.json` 回滚点；备份写入失败则整体中止导入并返回 500「导入前备份失败，为保护数据已中止导入」，保证零写入。「数据管理」面板「备份全量JSON」按钮仍调用 `backupBeforeImport()`，但仅作为**可选**的本地额外备份，不再是导入的前置条件。摘要返回 `backupFile` 供审计 |
...
3. ~~**备份下载**~~ ✅ **已拍板（用户 q-2）：导入强制先备份**。落地方式为**写时强制备份**——`POST /api/import/xlsx` 在写库前**自动**生成一份新的 `data/backup/backup-<ts>.json` 回滚点（`backupBeforeImport()` 内部完成）；备份写入失败则整体中止导入、返回 500「导入前备份失败，为保护数据已中止导入」并**零写入**。不要求管理员事先手动备份，避免出现"备份过期"或"忘记备份"导致的数据风险。「数据管理」面板「备份全量JSON」按钮保留为**可选的**本地额外备份工具。
